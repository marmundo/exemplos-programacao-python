...
def criar_venda(codigo: str, nome: str, valor: str, data: str) -> Venda:
    return Venda(codigo=int(codigo), nome=nome, valor=float(valor), data=datetime.strptime(data, "%d/%m/%Y"))


def salvar(venda: Venda) -> None:
    with open(NOME_ARQUIVO, "a", encoding="utf-8", newline="") as arquivo:
        escritor = csv.writer(arquivo, delimiter=";")
        escritor.writerow([venda.codigo, venda.nome, f"{venda.valor:.2f}", venda.data.strftime("%d/%m/%Y")])
